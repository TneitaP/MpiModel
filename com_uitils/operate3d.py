...
def catch_model2o3dmesh(pModel, paint_color_Arr = None, coord_mode = "xzy", model_format = "origin"):
    q_ani_mesh = o3d.geometry.TriangleMesh()
    if model_format == "origin":
        v_Arr = np.array(pModel.r)
        f_Arr = np.array(pModel.f)
    elif model_format == "np":
        v_Arr = np.array(pModel.verts)
        f_Arr = np.array(pModel.faces)
    if coord_mode == "xzy": 
        q_ani_mesh.vertices = o3d.utility.Vector3dVector(v_Arr[:,[ 0,2,1]]) # v_Arr[:,[ 0,2,1]]
    elif coord_mode == "xyz": 
        q_ani_mesh.vertices = o3d.utility.Vector3dVector(v_Arr) # v_Arr[:,[ 0,2,1]]
    q_ani_mesh.triangles = o3d.utility.Vector3iVector(f_Arr)
    q_ani_mesh.compute_vertex_normals()
    if paint_color_Arr is not None: 
        q_ani_mesh.paint_uniform_color(paint_color_Arr)
    return q_ani_mesh
